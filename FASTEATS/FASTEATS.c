...
int main()
{
    ...
    printf("Telefono: ");
    gets(telefono);
    fprintf(archivoPedido,"Teléfono     : %s\n",telefono);
    printf("Direccion: ");
    gets(direccion);
    fprintf(archivoPedido,"Dirección    : %s\n",direccion);
...
int j;
  ...
  do {
      ...
      if (cantidadOpcion != 0) {
         printf("   0.- Regresar al menu anterior\n");
         printf("    Opcion: ");
         scanf("%d", &opcionOpcion);

         if (opcionOpcion == 0)
            continue;
         else
            if (opcion[opcionOpcion-1+posicionOpcion].precio == 0)
               fprintf(archivoPedido,"   %s",
                       opcion[opcionOpcion-1+posicionOpcion].descripcion);
            else {
               fprintf(archivoPedido,"   %s\t\t\tEuros %.2lf\n",
                       opcion[opcionOpcion-1+posicionOpcion].descripcion,
                       opcion[opcionOpcion-1+posicionOpcion].precio);
               aPagar+=opcion[opcionOpcion-1+posicionOpcion].precio;
            }
      }
      else
         break;
